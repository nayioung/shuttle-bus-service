        ...
        <div className="bg-white rounded-[10px] overflow-hidden border border-[#D1D1D6]">
          {notices.map((notice, idx) => (
            <button 
              ...
              className="w-full flex items-center justify-between px-4 py-3.5 bg-white active:bg-[#D1D1D6] transition-colors text-left"
            >
              <div className="flex-1 min-w-0 pr-4">
                <h3 className="text-[16px] font-semibold text-black leading-tight truncate">
                  {notice.title}
                </h3>
                <p className="text-[12px] text-gray-400 mt-1 font-medium">{notice.date}</p>
              </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white max-w-[420px] mx-auto w-full">
      ...
      <div className="p-5 flex-1 overflow-y-auto">
        <div className="mb-6">
          <h1 className="text-[22px] font-black text-black leading-tight tracking-tight mb-2">
            {notice.title}
          </h1>
        ...
        <div className="text-[16px] text-black leading-[1.6] whitespace-pre-wrap font-medium">
          {notice.content}
        </div>
      </div>
    </div>
